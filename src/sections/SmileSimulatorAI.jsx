...
const SmileSimulatorAI = () => {
   ...
   */
  const enamelPixelForBracket = (mask, bw, lx, ly, data, w, imgH, minX, minY, relaxed) => {
    if (mask[ly * bw + lx] === 1) return true;
    if (relaxed) return pixelEnamelBracesLateral(data, w, imgH, minX + lx, minY + ly);
    return false;
  };
    ...
    for (let wx0 = xStart; wx0 <= xEnd - stripW + 1; wx0++) {
      ...
      for (let lx = wx0; lx <= wx1; lx++) {
        for (let ly = y0; ly <= y1; ly++) {
          if (!enamelPixelForBracket(mask, bw, lx, ly, data, w, imgH, minX, minY, relaxedEnamel)) continue;
  ...
  /** Collapse duplicate picks from adjacent tooth segments (keep brighter peak). */
  const dedupeCloseUpperStuds = (row, minDxPx) => {
    if (row.length === 0) return [];
    const sorted = [...row].sort((a, b) => a.cx - b.cx);
    const out = [];
    for (const p of sorted) {
      const last = out[out.length - 1];
      if (last && Math.abs(p.cx - last.cx) < minDxPx) {
        if ((p.peakMetric ?? 0) > (last.peakMetric ?? 0)) {
          out[out.length - 1] = { cx: p.cx, cy: p.cy, area: p.area ?? 1, peakMetric: p.peakMetric };
        }
      } else {
        out.push({ cx: p.cx, cy: p.cy, area: p.area ?? 1, peakMetric: p.peakMetric });
      }
    }
    return out;
  };
  ...
  const orderAndDedupeArchStuds = (row, archLx0, archLx1, maxBrackets, expectedSlots = null) => {
    ...
    for (const p of sorted) {
      const last = out[out.length - 1];
      if (last && Math.abs(p.cx - last.cx) < minSep) {
        if ((p.area ?? 1) >= (last.area ?? 1)) out[out.length - 1] = { ...p, area: p.area ?? 1 };
        continue;
      }
      out.push({ ...p, area: p.area ?? 1 });
    }
    if (out.length <= maxBrackets) return out;
    return subsampleArchRowToCount(out, maxBrackets);
  };
  ...
  const refineArchRowHorizontalEnamelCOM = (row, mask, bw, bh, minX, minY, halfWin = 9, bandHalfY = 6, minMaskPx = 6) => {
    ...
    const out = row.map((p) => {
      ...
      const lyC = clamp(Math.round(p.cy - minY), 0, bh - 1);
      const x0 = clamp(lxC - halfWin, 0, bw - 1);
      const x1 = clamp(lxC + halfWin, 0, bw - 1);
      const y0 = clamp(lyC - bandHalfY, 0, bh - 1);
      const y1 = clamp(lyC + bandHalfY, 0, bh - 1);
      let sumLX = 0;
      let cnt = 0;
  ...
  const applySmileCurveVerticalToArchRow = (row, isUpper, oval, depthScale = 1) => {
    ...
    return row.map((r) => {
      ...
      const dy = isUpper ? depth * q : -depth * q;
      return { ...r, cy: r.cy + dy };
    });
  };
  ...
  /**
   * Strict enamel only (TEETH_WHITEN-style): left/right gx in band — wire endpoints must not use lateral bleed.
   */
  const fullImageStrictTeethGxExtent = (data, w, h, yLoImg, yHiImg) => {
    const y0 = clamp(Math.min(yLoImg, yHiImg), 0, h - 1);
    const y1 = clamp(Math.max(yLoImg, yHiImg), 0, h - 1);
    let gl = w;
    let gr = -1;
  ...
  /** (top+bottom)/2 of strict enamel in column gx, clamped by BRACKET_VERTICAL_FACE_SAFE_FRAC; image Y. */
  const strictTeethColumnFaceY = (data, w, h, gx, yLoImg, yHiImg) => {
    const y0 = clamp(Math.min(yLoImg, yHiImg), 0, h - 1);
    const y1 = clamp(Math.max(yLoImg, yHiImg), 0, h - 1);
    if (gx < 1 || gx >= w - 1) return null;
    let top = 1e9;
    let bot = -1;
    for (let gy = y0; gy <= y1; gy++) {
      if (!pixelEnamelInTeethMask(data, w, h, gx, gy)) continue;
    ...
    const midRaw = (top + bot) * 0.5;
    return clampBracketMidYFrac(midRaw, top, bot, BRACKET_VERTICAL_FACE_SAFE_FRAC);
  };

  const strictTeethColumnFaceYNearest = (data, w, h, gx, yLoImg, yHiImg, maxD) => {
    const dMax = clamp(maxD | 0, 0, 12);
    for (let d = 0; d <= dMax; d++) {
      const xm = gx - d;
      if (xm >= 1) {
        const y = strictTeethColumnFaceY(data, w, h, xm, yLoImg, yHiImg);
        if (y != null) return y;
      }
      const xp = gx + d;
      if (d > 0 && xp < w - 1) {
        const y2 = strictTeethColumnFaceY(data, w, h, xp, yLoImg, yHiImg);
  ...
  /**
   * Catmull control polyline: strict enamel column centers at L/R + stud row between (no lip geometry).
   */
  const mergeWirePointsWithAnatomicalEnds = (studPtsImg, data, w, h, yLoImg, yHiImg) => {
    const sorted = [...studPtsImg].filter((p) => Number.isFinite(p.x) && Number.isFinite(p.y)).sort((a, b) => a.x - b.x);
    const ext = fullImageStrictTeethGxExtent(data, w, h, yLoImg, yHiImg);
    if (!ext || sorted.length === 0) return sorted;

    const xL = ext.L + 0.5;
    const xR = ext.R + 0.5;
    if (xR - xL < 4) return sorted;

    let yL = strictTeethColumnFaceY(data, w, h, ext.L, yLoImg, yHiImg);
    if (yL == null) yL = strictTeethColumnFaceYNearest(data, w, h, ext.L, yLoImg, yHiImg, 6);
    if (yL == null) yL = sorted[0].y;

    let yR = strictTeethColumnFaceY(data, w, h, ext.R, yLoImg, yHiImg);
    if (yR == null) yR = strictTeethColumnFaceYNearest(data, w, h, ext.R, yLoImg, yHiImg, 6);
    if (yR == null) yR = sorted[sorted.length - 1].y;

    const left = { x: xL, y: yL };
    const right = { x: xR, y: yR };
    const inner = sorted.filter((p) => p.x > xL + 2 && p.x < xR - 2);
    const body = inner.length > 0 ? inner : sorted.filter((p) => p.x >= xL && p.x <= xR);
  ...
  /** Left/right lx bounds of mask==1 within a horizontal band (enamel span for arch stretch). */
  const enamelLxExtentInBand = (mask, bw, bh, yLo, yHi) => {
    const y0 = clamp(Math.min(yLo, yHi), 0, bh - 1);
    const y1 = clamp(Math.max(yLo, yHi), 0, bh - 1);
    let L = bw;
    let R = -1;
    for (let ly = y0; ly <= y1; ly++) {
      const row = ly * bw;
      for (let lx = 0; lx < bw; lx++) {
        if (mask[row + lx] !== 1) continue;
        if (lx < L) L = lx;
        if (lx > R) R = lx;
      }
    }
  ...
  const mergeMorphSegmentsToMax = (segments, maxN) => {
    let segs = segments.filter(([a, b]) => b >= a && b - a + 1 >= MORPH_MIN_SEGMENT_WIDTH_PX);
    if (segs.length === 0) return segments;
    if (segs.length <= maxN) return segs;
    while (segs.length > maxN) {
      let bestI = 0;
      let bestW = Infinity;
      for (let i = 0; i < segs.length - 1; i++) {
        const wv = segs[i][1] - segs[i][0] + 1 + (segs[i + 1][1] - segs[i + 1][0] + 1);
        if (wv < bestW) {
          bestW = wv;
          bestI = i;
        }
      }
      const a0 = segs[bestI][0];
      const b1 = segs[bestI + 1][1];
      segs = [...segs.slice(0, bestI), [a0, b1], ...segs.slice(bestI + 2)];
    }
    return segs;
  };

  /**
   * True tooth-face center: horizontal + vertical centroid of mask pixels in segment (not segment mid-X).
   */
  const bracketCenterForToothSegment = (mask, bw, minX, minY, xL, xR, y0, y1) => {
    let sumX = 0;
    let sumY = 0;
    let cnt = 0;
    let topY = 1e9;
    let botY = -1;
    for (let lx = xL; lx <= xR; lx++) {
      for (let ly = y0; ly <= y1; ly++) {
        if (mask[ly * bw + lx] !== 1) continue;
        sumX += lx + 0.5;
        sumY += ly + 0.5;
    ...
    if (cnt < MORPH_MIN_TOOTH_BLOB_PX) return null;
    const midLx = sumX / cnt;
    const midLy = sumY / cnt;
    const midClamped = clampBracketMidYFrac(midLy, topY, botY, BRACKET_VERTICAL_FACE_SAFE_FRAC);
    const cx = minX + midLx;
    const cy = minY + midClamped;
  ...
  const estimateToothSlotCount = (lx0, lx1, minSlots, maxSlots) => {
    ...
    return clamp(Math.round(span / EST_TOOTH_COL_WIDTH_PX), minSlots, maxSlots);
  };

  /**
   * Morphological tooth lock: per-column enamel mass in band → vertical valleys → one segment per tooth;
   * one anchor at segment COM. Upper/lower call separately (no X mirroring).
   */
  const buildMorphologicalArchRow = (
    mask,
    bw,
    bh,
    data,
    ...
    const nCol = archLx1 - archLx0 + 1;
    if (nCol < 6) return [];

    const raw = new Float64Array(nCol);
    for (let k = 0; k < nCol; k++) {
      const lx = archLx0 + k;
      let cnt = 0;
      for (let ly = y0; ly <= y1; ly++) {
        if (mask[ly * bw + lx] === 1) cnt++;
      }
      raw[k] = cnt;
    }

    const blurR = MORPH_SIGNAL_BLUR_HALF;
    const sm = new Float64Array(nCol);
    for (let k = 0; k < nCol; k++) {
      let s = 0;
      let c = 0;
      for (let d = -blurR; d <= blurR; d++) {
        const kk = k + d;
        if (kk < 0 || kk >= nCol) continue;
        s += raw[kk];
        c++;
      }
      sm[k] = c > 0 ? s / c : 0;
    ...
    let maxSm = 0;
    for (let k = 0; k < nCol; k++) if (sm[k] > maxSm) maxSm = sm[k];

    const edge = MORPH_EDGE_VALLEY_IGNORE_PX;
    /** >40% drop vs neighbors when frac=0.6 (sm[k] < 0.6 × neighbor average). */
    const isNeighborValley = (k, frac) => {
      if (k < 1 || k >= nCol - 1) return false;
      const nbr = (sm[k - 1] + sm[k + 1]) * 0.5;
      if (nbr < 0.5) return false;
      if (sm[k] > nbr * frac) return false;
      return sm[k] < sm[k - 1] && sm[k] < sm[k + 1];
    };

    const valleyLxList = [];
    for (let k = 1; k < nCol - 1; k++) {
      if (k < edge || k >= nCol - edge) {
        if (sm[k] > maxSm * 0.2) continue;
      }
      if (!isNeighborValley(k, MORPH_VALLEY_NEIGHBOR_MAX_FRAC)) continue;
      valleyLxList.push(archLx0 + k);
    }

    valleyLxList.sort((a, b) => a - b);
    const mergedValleys = [];
    for (const vlx of valleyLxList) {
      const prev = mergedValleys[mergedValleys.length - 1];
      if (prev != null && vlx - prev < 4) {
        const iPrev = prev - archLx0;
        const iV = vlx - archLx0;
        if (sm[iV] < sm[iPrev]) mergedValleys[mergedValleys.length - 1] = vlx;
      } else mergedValleys.push(vlx);
    }

    const buildSegmentsFromValleys = (valleys) => {
      const segs = [];
      let xStart = archLx0;
      for (const cut of valleys) {
        const xEnd = cut - 1;
        if (xEnd >= xStart && xEnd - xStart + 1 >= MORPH_MIN_SEGMENT_WIDTH_PX) {
          segs.push([xStart, xEnd]);
        }
        xStart = cut + 1;
      }
      if (archLx1 >= xStart && archLx1 - xStart + 1 >= MORPH_MIN_SEGMENT_WIDTH_PX) {
        segs.push([xStart, archLx1]);
      }
      return segs;
    };

    let segments = buildSegmentsFromValleys(mergedValleys);

    if (segments.length <= 1 && nCol >= 28) {
      const merged2 = [...mergedValleys];
      for (let k = edge + 2; k < nCol - edge - 2; k++) {
        if (!isNeighborValley(k, MORPH_VALLEY_NEIGHBOR_LOOSE_FRAC)) continue;
        merged2.push(archLx0 + k);
      }
      merged2.sort((a, b) => a - b);
      const dedup = [];
      for (const v of merged2) {
        const last = dedup[dedup.length - 1];
        if (last != null && v - last < 4) continue;
        dedup.push(v);
      }
      segments = buildSegmentsFromValleys(dedup);
    }
    ...
    /** Valleys missed: one segment across molars — subdivide by ~EST_TOOTH_COL_WIDTH_PX so count tracks arch width. */
    if (mergedSegs.length === 1) {
      const [a, b] = mergedSegs[0];
      const span = b - a + 1;
      const minF = isLower ? MIN_BRACKETS_LOWER_FALLBACK : MIN_BRACKETS_UPPER_FALLBACK;
      if (span >= 24) {
        const n = estimateToothSlotCount(a, b, minF, maxBrackets);
        if (n > 1) {
          const pieces = [];
          for (let i = 0; i < n; i++) {
            const x0 = a + Math.floor((i * span) / n);
            const x1 = i === n - 1 ? b : a + Math.floor(((i + 1) * span) / n) - 1;
            if (x1 >= x0 && x1 - x0 + 1 >= MORPH_MIN_SEGMENT_WIDTH_PX) pieces.push([x0, x1]);
          }
          if (pieces.length >= 2) mergedSegs = pieces;
        }
      }
    }
  ...
  const buildCentroidBracesPack = (landmarks, iw, ih, oval, enamelSnap) => {
    ...
    const stripY0Pre = polyMinY - archPadY;
    const stripY1Pre = polyMaxY + archPadY;
    const fullXExtent = fullImageEnamelGxExtent(
      data,
      w,
      h,
    ...
    const xPadGeom = Math.max(40, (polyMaxX - polyMinX) * 0.15);
    /** Horizontal ROI from absolute enamel L/R only (inner lip polygon does not clip width). */
    let minX;
    let maxX;
    if (fullXExtent) {
      minX = Math.floor(clamp(fullXExtent.L - xPadGeom, 0, w - 2));
      maxX = Math.ceil(clamp(fullXExtent.R + xPadGeom, minX + 2, w - 1));
    } else {
      minX = Math.floor(clamp(polyMinX - archPadX, 0, w - 2));
      maxX = Math.ceil(clamp(polyMaxX + archPadX, minX + 2, w - 1));
    }
    const minY = Math.floor(clamp(polyMinY - 2, 0, h - 2));
    const maxY = Math.ceil(clamp(polyMaxY + 2, minY + 2, h - 1));
    ...
    const imgYLowerLo = clamp(minY + yLowerStart, 0, h - 1);
    const imgYLowerHi = clamp(minY + bh - 1, 0, h - 1);

    const globUpper = globalEnamelExtentLocalX(data, w, h, imgYUpperLo, imgYUpperHi, minX, bw);
    const globLower = globalEnamelExtentLocalX(data, w, h, imgYLowerLo, imgYLowerHi, minX, bw);
    const extUpperImg = fullImageEnamelGxExtent(data, w, h, imgYUpperLo, imgYUpperHi);
    const extLowerImg = fullImageEnamelGxExtent(data, w, h, imgYLowerLo, imgYLowerHi);

    let archLx0_u = uBand.L;
    let archLx1_u = uBand.R;
    if (globUpper) {
      archLx0_u = Math.min(archLx0_u, globUpper.locL);
      archLx1_u = Math.max(archLx1_u, globUpper.locR);
    }
    if (extUpperImg) {
      const lLoc = clamp(Math.floor(extUpperImg.L - minX), 0, bw - 1);
      const rLoc = clamp(Math.ceil(extUpperImg.R - minX), 0, bw - 1);
      archLx0_u = Math.min(archLx0_u, lLoc);
      archLx1_u = Math.max(archLx1_u, rLoc);
    }

    let archLx0_l = lBand.L;
    let archLx1_l = lBand.R;
    if (globLower) {
      archLx0_l = Math.min(archLx0_l, globLower.locL);
      archLx1_l = Math.max(archLx1_l, globLower.locR);
    }
    if (extLowerImg) {
      const lLocL = clamp(Math.floor(extLowerImg.L - minX), 0, bw - 1);
      const rLocL = clamp(Math.ceil(extLowerImg.R - minX), 0, bw - 1);
      archLx0_l = Math.min(archLx0_l, lLocL);
      archLx1_l = Math.max(archLx1_l, rLocL);
    }
    ...
    const nUpperTarget = estimateToothSlotCount(
      morphArch0_u,
      morphArch1_u,
      MIN_BRACKETS_UPPER_FALLBACK,
      MORPH_MAX_UPPER_BRACKETS,
    );
    const nLowerTarget = estimateToothSlotCount(
      morphArch0_l,
      morphArch1_l,
      MIN_BRACKETS_LOWER_FALLBACK,
