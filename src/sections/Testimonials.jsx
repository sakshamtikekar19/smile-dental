...
const Testimonials = () => {
  return (
    <section id="gallery" className="py-24 md:py-32 bg-[#030303] overflow-hidden scroll-mt-28 relative">
      {/* Background patterns */}
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 md:gap-24 items-center">
          <AnimatedSection>
            ...
            <div className="space-y-10 md:space-y-16 mt-12 md:mt-20">
              {testimonials.map((t) => (
                <div key={t.name} className="group flex gap-5 md:gap-10 items-start transition-all">
                  ...
                  <div className="flex-1 pt-2">
                    <p className="text-lg md:text-2xl text-[#808080] font-serif italic mb-5 md:mb-8 leading-relaxed relative group-hover:text-[#A0A0A0] transition-colors">
          ...
          <AnimatedSection delay={0.3} className="relative">
            <div className="grid grid-cols-2 gap-6">
              {galleryImages.map((img, i) => (
                <motion.div 
                  key={i}
                  whileHover={{ y: -15 }}
                  className={`relative rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/5 ${i % 2 === 1 ? "translate-y-8 md:translate-y-16" : ""}`}
                >
                  <img 
                    src={img} 
            ...
            <div className="absolute -bottom-10 md:-bottom-16 right-0 md:-right-8 z-20 glass-medical p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl max-w-[200px] md:max-w-[240px] glow-combined">
