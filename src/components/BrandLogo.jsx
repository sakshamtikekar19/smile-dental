...
const BrandLogo = ({ className = "", collapsed = false }) => {
  return (
    <div className={`flex items-center gap-2.5 group cursor-pointer ${className}`}>
      ...
      >
        <div className="absolute inset-0 bg-accent-blue rounded-xl rotate-45 group-hover:rotate-0 transition-transform duration-500 shadow-lg shadow-accent-blue/20" />
        <div className="absolute inset-1 bg-white rounded-lg rotate-45 group-hover:rotate-0 transition-transform duration-500" />
        <svg 
          viewBox="0 0 24 24" 
          className="relative z-10 w-5 h-5 text-accent-blue"
          fill="none" 
          stroke="currentColor" 
          strokeWidth="2.5" 
          strokeLinecap="round" 
      ...
      {!collapsed && (
        <div className="flex flex-col">
          ...
          <span className="text-[9px] uppercase tracking-[0.3em] font-bold text-text-secondary mt-1 leading-none">
